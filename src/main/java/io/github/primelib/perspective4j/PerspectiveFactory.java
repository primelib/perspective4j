...
import io.github.primelib.perspective4j.auth.AuthInterceptor;
...
public class PerspectiveFactory {
    ...
    public static <T> T create(Consumer<PerspectiveFactorySpec<T>> spec) {
        ...
        if (config.api().isInterface()) {
            ...
            try {
                PerspectiveApi api = buildClient(s -> {
                    s.api(PerspectiveApi.class);
                    s.applySpec(config);
                });
                return config.api().getConstructor(PerspectiveApi.class).newInstance(api);
            } catch (Exception ex) {
                throw new IllegalArgumentException("api must have a constructor with one parameter of type PerspectiveApi", ex);
            }
        }
    }
